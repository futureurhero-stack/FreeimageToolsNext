...
import styles from "../../app/styles/variations/design1.module.css";
...
Our JPG converter runs 100% in your browser, so there's no need to upload images to a server. This is a significant advantage in terms of privacy and security. Converted images are immediately available for download and use.`
  ...
  // ---------- drop handlers (identical to source) ----------
  ...
  // ---------- UI (with CSS module classes) ----------
  return (
    <div className={styles.wrapper}>
      <header className={styles.siteHeader}>
        <div className={styles.brand}>
          <Link href="/" className={styles.brandLink}>
          <div className={styles.logoCircle}>FI</div>
        ...
        <div className={styles.topRight}>
          ...
          <div className={styles.heroNote}>{texts.heroNote}</div>
        </div>
      </header>

      <nav className={styles.pageNav}>
        <Link href="/" className={`${styles.navLink} ${styles.active}`}>{texts.nav.jpg}</Link>
        <Link href="/resize" className={styles.navLink}>{texts.nav.resize}</Link>
        <Link href="/idphoto" className={styles.navLink}>{texts.nav.idphoto}</Link>
      </nav>

      <main className={styles.card}>
        <section className={styles.panel}>
          <div className={styles.panelHeader}>
            <div className={styles.panelTitle}>{texts.jpg.title}</div>
            <div className={styles.panelSub}>{texts.jpg.sub}</div>
          </div>
          ...
          <div className={styles.optionBlock}>
            ...
            <div className={styles.optionRow}>
              ...
              />
              <span className={styles.sliderValue}>{jpgQuality}</span>
            </div>
          </div>

          <div className={styles.dropRow}>
            <div
              className={`${styles.dropZone} ${jpgDrag ? styles.dragover : ""}`}
            ...
            >
              <div className={styles.dropMain}>{texts.jpg.dropMain}</div>
              <div className={styles.dropSub}>{texts.jpg.dropSub}</div>
              <input
                type="file"
                ref={jpgFileInputRef}
                style={{ display: 'none' }}
            ...
            <div className={styles.statusBox}>
              <div className={styles.statusTitle}>{texts.status}</div>
              {jpgStatus}
            </div>
          </div>
        </section>
      ...
      <footer className={styles.footer}>© {year} {texts.footer}</footer>
    </div>
  );
}
